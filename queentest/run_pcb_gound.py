...
for number in range(start,start+1):
    ...
    for numberblocked in range(s,e+1):
        mystring="gringo blocked" +str(numberblocked)+".lp "+str(file_name1)+ ".lp --const n=" +str(start)+" > a.grd"
        mystring1="clasp a.grd"
        print mystring
        cout=0 
        t=0
        #cmd="./blocked "+str(numberblocked)+" "+str(number)
        #os.system(cmd)
        os.popen(mystring).read()
        start_time = time.time() 
        x=os.popen(mystring1).read() # This will run the command and return any outp
