...
def CreateRandomSubBoard(filen,filenumber, sizeofn, dimisionsBoard):
    temp = []
    listOfBlocked = []
    blocked_file = open(filen,"r")
    ...
    for x in numbers:
        if beginSub_0 <= x[0] <= endSub_0:
            if beginSub_1 <= x[1] <= endSub_1:
                SubBoard.append(x)
    #this is where you want to save the thing
    ...
    for x in SubBoard:
        ...
        ##change the blocked( to what ever you have. 
    ...
    f.close()
    #print(SubBoard)
#print(CreateRandomSubBoard("./blocked/blocked_n=60_3111",3459,70,20))